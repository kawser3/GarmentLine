...
| The same defect recurs on one line and nobody notices until the third time | Every raise — manual or AI-accepted — runs `detectRepeat` over `Issue.CreatedDate` for the same `Type` and `LineId`. At two occurrences inside ten days a `RepeatAlert` row is written or updated and the factory manager is notified in-app | `/issues/new`, alert table on `/dashboard` |
